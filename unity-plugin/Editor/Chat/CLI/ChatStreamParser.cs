    ...
    public static class ChatStreamParser
    {
        ...
        private static ChatEvent? ParseInternal(string line)
        {
            ...
            switch (type)
            {
                ...
                case "control_request":      return ParseControlRequest(line);
                case "control_response":     return null; // initialize response, silently ignored
                case "sdk_control_request":  return ParseControlRequest(line); // backward compat
                case "tool_progress":        return ParseToolProgress(line);
                case "rate_limit_event":     return ParseRateLimit(line);
                case "session_state_changed": return ParseSessionState(line);
        ...
        // ── new event types (Sprint 1A) ───────────────────────────────────────

        private static ChatEvent? ParseControlRequest(string line)
        {
            var request = JsonHelper.ExtractObject(line, "request");
            var subtype = JsonHelper.ExtractString(request, "subtype");
            switch (subtype)
            {
                case "can_use_tool":
                    // Agent SDK protocol: request_id, tool_name, input all inside request object
                    var cuReqId    = JsonHelper.ExtractString(request, "request_id");
                    var cuToolName = JsonHelper.ExtractString(request, "tool_name");
                    var cuInput    = JsonHelper.ExtractObject(request, "input");
                    if (cuToolName == "AskUserQuestion")
                        return ChatEvent.AskUser(cuReqId, cuInput);
                    return ChatEvent.PermissionPrompt(cuReqId, cuToolName, cuInput);
                case "hook_callback":
                    // Old hooks protocol (v2.1.177): request_id is at top level, tool info inside request.input
                    var reqId    = JsonHelper.ExtractString(line, "request_id");
                    var input    = JsonHelper.ExtractObject(request, "input");
                    var toolName = JsonHelper.ExtractString(input, "tool_name");
                    var toolInp  = JsonHelper.ExtractObject(input, "tool_input");
                    // AskUserQuestion has no separate elicitation event in hooks protocol
                    if (toolName == "AskUserQuestion")
                        return ChatEvent.AskUser(reqId, toolInp);
                    return ChatEvent.PermissionPrompt(reqId, toolName, toolInp);
                case "permission":
                    // Old protocol (backward compat)
                    var pReqId    = JsonHelper.ExtractString(request, "request_id");
                    var pToolName = JsonHelper.ExtractString(request, "tool_name");
                    var pToolInp  = JsonHelper.ExtractObject(request, "tool_input");
                    return ChatEvent.PermissionPrompt(pReqId, pToolName, pToolInp);
                case "elicitation":
                    var eReqId  = JsonHelper.ExtractString(request, "request_id");
                    var rawJson = JsonHelper.ExtractObject(request, "elicitation");
                    return ChatEvent.AskUser(eReqId, rawJson);
                default:
                    return null; // mcp_message, initialize response, and unknown subtypes = no-op
            }
        }

        private static ChatEvent? ParseToolProgress(string line)
        {
            var pctStr = JsonHelper.ExtractString(line, "percentage");
            float.TryParse(pctStr, System.Globalization.NumberStyles.Float,
                System.Globalization.CultureInfo.InvariantCulture, out var pct);
            var msg = JsonHelper.ExtractString(line, "message");
            return ChatEvent.ToolProgress(pct, msg);
        }

        private static ChatEvent? ParseRateLimit(string line)
        {
            var msg = JsonHelper.ExtractString(line, "message") ?? "Rate limited";
            return ChatEvent.RateLimit(msg);
        }

        private static ChatEvent? ParseSessionState(string line)
        {
            var state = JsonHelper.ExtractString(line, "state") ?? "";
            return ChatEvent.SessionState(state);
        }
    }
}
