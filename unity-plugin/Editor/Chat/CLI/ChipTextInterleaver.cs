            ...
            foreach (var pc in sorted)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        // Advance past @mention so it doesn't leak into cleanText.
                        string mentionOnly = "@" + pc.Chip.DisplayName;
                        if (rawPos + mentionOnly.Length <= rawText.Length &&
                            string.Compare(rawText, rawPos, mentionOnly, 0, mentionOnly.Length,
                                System.StringComparison.Ordinal) == 0)
                        {
                            rawPos += mentionOnly.Length;
                            if (rawPos < rawText.Length && rawText[rawPos] == ' ') rawPos++;
                        }
                    }
                }
            }
