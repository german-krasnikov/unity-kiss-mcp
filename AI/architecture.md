...
### Agent Chat Backends (C#: CliBackendBase + subclasses, v0.14.0+, v0.53.1: Codex Elicitation Defensiveness, v0.55.0: External MCP + Scoped Config)
- **CliBackendBase** (abstract, v0.55.0: port via scoped config): Shared lifecycle host for CLI-based backends. 4 variation axes: (a) `BuildArgs` (spawn/resume argv + env-key-strip), (b) `ParseLine` (NDJSON line → ChatEvent[]), (c) `BinaryName` (CLI executable), (d) `IsPersistentProcess` (stdin loop vs spawn-per-turn). **BREAKING v0.55.0 RULE:** `BuildSpawnEnv()` returns ONLY `{ "UNITY_MCP_SESSION_TIMEOUT": "300" }` — NEVER injects UNITY_MCP_PORT into process env. Instead, each backend delivers port via scoped --mcp-config: the config file contains `"environment": { "UNITY_MCP_PORT": "<port>" }` in its "mcp"/"unity-mcp" block. This prevents env var leakage to ~/.claude/mcp.json's unity-kiss-mcp entry (which would spawn duplicate bridge on wrong port). Owns spawn, drain, accumulate, SessionId, Stop, Dispose — subclasses override only the 4 axes. **v0.53.1:** Routes `ChatEvent.autoReply` (AutoReply enum) decisions to stdout for inbound server-request auto-responses (transparent to UI).
