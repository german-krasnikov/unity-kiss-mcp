...
### Agent Chat Backends (C#: CliBackendBase + subclasses, v0.14.0+, v0.53.1: Codex Elicitation Defensiveness, v0.55.0: External MCP + Scoped Config, v0.64.0: SessionId Resume)
- **CliBackendBase** (abstract, v0.55.0: port via scoped config, v0.64.0: eager SessionId persist): Shared lifecycle host for CLI-based backends. 4 variation axes: (a) `BuildArgs` (spawn/resume argv + env-key-strip), (b) `ParseLine` (NDJSON line → ChatEvent[]), (c) `BinaryName` (CLI executable), (d) `IsPersistentProcess` (stdin loop vs spawn-per-turn). **BREAKING v0.55.0 RULE:** `BuildSpawnEnv()` returns ONLY `{ "UNITY_MCP_SESSION_TIMEOUT": "300" }` — NEVER injects UNITY_MCP_PORT into process env. Instead, each backend delivers port via scoped --mcp-config: the config file contains `"environment": { "UNITY_MCP_PORT": "<port>" }` in its "mcp"/"unity-mcp" block. This prevents env var leakage to ~/.claude/mcp.json's unity-kiss-mcp entry (which would spawn duplicate bridge on wrong port). Owns spawn, drain, accumulate, SessionId, Stop, Dispose — subclasses override only the 4 axes. **v0.53.1:** Routes `ChatEvent.autoReply` (AutoReply enum) decisions to stdout for inbound server-request auto-responses (transparent to UI). **T6 (v0.64.0): SessionId Eager Persist** — SessionId written to SessionState immediately after generation (not lazy on turn). Fallback: reload-safe SessionId recovery via SessionState lookup. Enables Kimi graceful degradation on domain reload (preserves session context).
- **AutoReply Routing (v0.53.1)**: **ChatEvent.cs** adds `AutoReply` enum {None, CodexElicitationAccept, CodexElicitationDecline} — signals CliBackendBase when to auto-reply to inbound Codex app-server requests without user interaction. CliBackendBase checks this field before emission; if autoReply ≠ None, writes JSON-RPC response to stdout (suppresses tool approval card UI, auto-accepts/declines constraint-matching decisions).
- **T6 (v0.64.0): Kimi Session Resume** — `KimiBackend` now implements session resume via SessionId persistence. On domain reload, SessionId restored from SessionState. Graceful degradation: missing SessionId falls back to new session (no error thrown). `KimiParserTests` extended with 51+ assertions for resume edge cases.
- **Request vs Notification Dispatch (v0.53.1)**: **CodexAppServerParser.cs DispatchNotification** now distinguishes top-level `id` field presence: requests have numeric `id` (require JSON-RPC response), notifications absent (fire-and-forget). `HasRpcId()` helper checks for field presence via depth-aware `JsonHelper.ExtractString(line, "id")` (returns top-level keys only, ignores nested `id` in params); requests route to auto-reply queuing, notifications to ChatEvent emission. Prevents silent loss of approval-blocking requests and prevents false positive on `turn/started` notification with nested `params.turn.id`.
- **ControlResponseBuilder DRY (v0.53.1)**: Extracted `FormatRpcId(id)` static method reused by both `CodexElicitationAccept()` and `CodexUserInputResponse()` — formats numeric id as unquoted, string id as quoted in JSON-RPC response. Centralizes id-type safety guard (validates `int.TryParse()`), preventing response format corruption on mixed id types.
- **Layer-1 Sandbox/Approval Fields (v0.53.1)**: **CodexAppServerBackend.cs** now includes approval suppression fields in `thread/start` and `turn/start` payloads: `approvalPolicy` (granular `mcp_elicitations:false`) + `sandbox` (string "danger-full-access") + `sandboxPolicy` (object {type:"dangerFullAccess"}). These suppress Codex elicitation requests at source (perf optimization), reducing reliance on Layer 2/3 auto-reply.
- **ClaudeBackend** (ported onto base, v0.55.0: external MCP additivity): Zero behavior change (−65 lines net). Persistent stdin loop (IsPersistentProcess=true), Claude NDJSON parser, `--resume <sessionId>` argv builder. Uses `ChatMcpConfigWriter.GetOrCreateConfigPath()` to generate temporary JSON config + `--mcp-config <path>` flag. No `--strict-mcp-config` (v0.38.0) — Claude CLI automatically merges our MCP config with user's `~/.claude/` servers (Blender MCP, luna-kiss-mcp, etc.). **v0.55.0:** UNITY_MCP_PORT delivered in config env block (never process env); legacy env fallback removed.
- **CodexAppServerBackend** (only Codex option, v0.14.0+, v0.53.1: elicitation defensive, v0.55.0: scoped port delivery): Persistent Codex session via `codex app-server` (direct stdio, JSON-RPC 2.0). One process per chat session (IsPersistentProcess=true). Protocol: `initialize` → `thread/start` → repeated `turn/start` with `mcpToolCall` items + real token streaming via `item/agentMessage/delta` (240+ deltas/turn). MCP injection via `-c mcp_servers.*` flags at session init. **v0.53.1:** Processes inbound `mcpServer/elicitation/request` (top-level `id` field) to auto-accept matching constraints; emits AutoReply=CodexElicitationAccept to suppress UI card and auto-submit response via stdout. Spike-verified with codex 0.137.0–0.141.0. **v0.55.0:** UNITY_MCP_PORT delivered in scoped config env block (not process env).
- **CodexAppServerParser** (v0.53.1): JSON-RPC 2.0 notification/response parser → ChatEvent. Now distinguishes notifications vs requests via HasRpcId check (top-level `id` only). Notifications (absent id): mcpToolCall camelCase, agentMessage/delta token stream, turn/completed with usage; thread_id at result.thread.id. Requests (numeric id): routed to auto-reply response queue for ControlResponseBuilder.
...
- **External MCP Additivity (v0.55.0, OpenCode):** OpenCode backend now merges 3rd-party MCP entries from global config into the scoped config's "mcp" block. **OpenCodeArgBuilder.MergeGlobalOpenCodeConfig()**: (1) reads ~/.opencode/config.json, (2) locates "mcp" block via brace-depth walk, (3) filters entries by key (IsNonUnityKey excludes "unity", "unity-mcp", "unity-kiss-mcp" keys to prevent conflicts), (4) extracts object-valued entries only (arrays/strings/primitives skipped — MCP servers are objects by spec), (5) injects filtered entries before closing brace of base config's "mcp" block via `JsonMergeHelper.InjectBeforeBlockClose()`. Result: Claude/Codex/etc. gain access to user's Blender MCP, other local tools additively (no allowlist, no strict segregation). **JsonMergeHelper utilities**: `FindBlockClose(json, key)` locates closing brace of named object, `ExtractObjectEntries(blockContent, keyFilter)` yields "key": {...} entries, `InjectBeforeBlockClose(baseJson, key, entries)` inserts entries + comma + indent before closing brace. Pure string surgery (no JSON parsing library), graceful on malformed input (returns original unchanged).
