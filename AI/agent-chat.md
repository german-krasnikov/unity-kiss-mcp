...
**Subprocess Environment (v0.36.0)** — CliBackendBase injects three env vars:
- **UNITY_MCP_PORT** — chat server port (explicit value, accelerates Python discovery)
- **UNITY_MCP_CHAT=1** — marker for Python bridge to use *.chat-port files (Windows fallback)
...
**v0.36.0: Env block injection** — `BuildClaudeConfigJson()` now emits `"env":{"UNITY_MCP_PORT":"<port>"}` block (when chat port > 0). Python bridge reads this from the config env and uses it for initial connection, falling back to discovery files if needed.

Port discovery chain: no explicit port embedded in config itself (server self-discovers via `~/.unity-mcp/ports/*.port` or *.chat-port), but env var accelerates connection. The hardcoded `~/.claude/mcp.json` path is now a fallback only (used if config generation fails). This eliminates the need for manual setup in most cases.

**Windows fallback (v0.36.0):** When UNITY_MCP_CHAT=1 env is set by C# subprocess, Python reads *.chat-port files instead of *.port (fallback when UNITY_MCP_PORT env propagation fails across process boundary).
