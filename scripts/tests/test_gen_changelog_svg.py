...
class TestParseChangelog:
    def test_real_changelog_returns_five_versions(self, real_changelog: str) -> None:
        """Test (1): real CHANGELOG has exactly 5 annotated versions."""
        versions = gen.parse_changelog(real_changelog)
        assert len(versions) == 5

    def test_real_changelog_oldest_first(self, real_changelog: str) -> None:
        """Test (2): oldest version (v0.2.6) is index 0, newest (v0.6.0) is index 4."""
        versions = gen.parse_changelog(real_changelog)
        assert versions[0].ver == "0.2.6"
        assert versions[4].ver == "0.6.0"

    def test_caption_from_annotation(self, real_changelog: str) -> None:
        """Test (3): caption comes from <!-- svg: ... --> annotation."""
        versions = gen.parse_changelog(real_changelog)
        # v0.6.0 has svg annotation
        newest = versions[4]
        assert newest.caption == "Aura pill + native theme + perms gating"
...
class TestLayout:
    def test_five_versions_produce_six_x_positions(self, real_changelog: str) -> None:
        """Test (9): 5 versions → 6 x-positions (5 real + 1 future)."""
        versions = gen.parse_changelog(real_changelog)
        layout = gen.layout(versions)
        assert len(layout.xs) == 6

    def test_x_positions_endpoints(self, real_changelog: str) -> None:
        """Test (9b): x[0]=140, x[5]=viewbox_w-120."""
        versions = gen.parse_changelog(real_changelog)
        lo = gen.layout(versions)
        assert lo.xs[0] == pytest.approx(140.0)
        assert lo.xs[5] == pytest.approx(lo.viewbox_w - 120.0)
    ...
    def test_opacity_ramp_monotonic(self, real_changelog: str) -> None:
        ...
        versions = gen.parse_changelog(real_changelog)
        lo = gen.layout(versions)
        ops = lo.opacities[:len(versions)]  # exclude future node
    ...
    def test_ring_counts(self, real_changelog: str) -> None:
        ...
        versions = gen.parse_changelog(real_changelog)
        lo = gen.layout(versions)
        for i, (op, rings) in enumerate(zip(lo.opacities[:len(versions)], lo.ring_counts[:len(versions)])):
...
class TestRenderSvg:
    ...
    def test_all_version_strings_appear(self, svg_5ver: str) -> None:
        """Test (19): every version string appears as text in SVG."""
        for ver in ["v0.2.6", "v0.3.0", "v0.4.0", "v0.5.0", "v0.6.0"]:
            assert ver in svg_5ver
...
class TestBaselinePath:
    def test_baseline_ends_at_w_minus_60(self, real_changelog: str) -> None:
        ...
        w = lo.viewbox_w
        wend = w - 60
        # The last L command in the baseline path should be L{wend} 240
        assert f"L{wend} 240" in svg, f"Expected baseline to end at L{wend} 240 (w={w})"
        # And specifically NOT stop at w//2 (the midpoint bug)
