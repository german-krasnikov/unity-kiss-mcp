"""Live integration test: search_scene with root scope.
...
async def test_live_search_scene_scoped(bridge):
    ...
    parent = f"Live_Scoped_Parent_{uuid.uuid4().hex[:6]}"
    child = f"Live_Scoped_Child_{uuid.uuid4().hex[:6]}"
    sibling = f"Live_Scoped_Sibling_{uuid.uuid4().hex[:6]}"
    ...
    try:
        result = await bridge.send("search_scene", {"query": "Live_Scoped_", "root": "/" + parent})
